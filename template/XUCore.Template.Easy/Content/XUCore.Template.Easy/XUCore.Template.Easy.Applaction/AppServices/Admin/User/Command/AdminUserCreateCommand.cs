        ...
        public class Validator : CommandValidator<AdminUserCreateCommand>
        {
            public Validator()
            {
                RuleFor(x => x.UserName).NotEmpty().MaximumLength(20).WithName("账号")
                    ...
                    {
                        var res = await Web.GetService<IAdminUserAppService>().GetAnyAsync(AccountMode.UserName, account, 0, cancel);
                ...
                RuleFor(x => x.Mobile)
                    ...
                    .MustAsync(async (account, cancel) =>
                    {
                        var res = await Web.GetService<IAdminUserAppService>().GetAnyAsync(AccountMode.Mobile, account, 0, cancel);
