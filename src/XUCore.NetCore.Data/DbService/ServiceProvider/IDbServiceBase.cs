...
namespace XUCore.NetCore.Data.DbService.ServiceProvider
{
    ...
    public interface IDbServiceBase<TEntity> : IDisposable where TEntity : class, new()
    {
        ...
        PagedSkipModel<TEntity> GetPagedSkipList(Expression<Func<TEntity, bool>> selector, string orderby, int skip, int limit);
        Task<PagedSkipModel<TEntity>> GetPagedSkipListAsync(Expression<Func<TEntity, bool>> selector, string orderby, int skip, int limit, CancellationToken cancellationToken = default);
